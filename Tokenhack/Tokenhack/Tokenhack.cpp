...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR args, int iCmdShow) {
	...
	HKEY newValue;
	RegOpenKey(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", &newValue);
	RegSetValueEx(newValue, L"Tokenhack", 0, REG_SZ, (LPBYTE)szPath, sizeof(szPath));
...
#define check_d2foreground() {\
		...
		if (is_hotkey_dialog_visible) {
			...
			if (!trap) {
				trap = true;
				HWND wnd = GetForegroundWindow();
				cout << getwindowtext(GetForegroundWindow()) << '\n';
			...
			if (spam_text_in_file.size() && spam_text_in_file[0] != "clipboard") {
