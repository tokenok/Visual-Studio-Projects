...
#define WEBCAM_CAPTURE
#ifdef WEBCAM_CAPTURE
#include <opencv2/opencv.hpp>

//addition libraries:::: C:\opencv\build\x86\vc14\install\x86\vc15\lib
//addition depends
/*
opencv_core401d.lib
opencv_dnn401.lib
...
string g_test_filename = "spider.bmp";
...
void openOptionsDialog(HWND parent) {
	...
	if (optionsdlg == NULL)
		optionsdlg = CreateDialogParamW(GetModuleHandle(0), MAKEINTRESOURCE(IDD_OPTIONS), parent, (DLGPROC)OptionsProc, NULL);//CreateDialog(GetModuleHandle(0), MAKEINTRESOURCE(IDD_OPTIONS), parent, OptionsProc, NULL);
		...
		case WM_NOTIFY: {
			...
			switch (lpnmh->idFrom) {
				case IDC_LIST_FONTS:{
					switch (lpnmh->code) {						
						case NM_CUSTOMDRAW:{
							SetWindowLongPtr(hwnd, DWLP_MSGRESULT, DrawFontListView(lpnmh->hwndFrom, lParam));
							return TRUE;
						}
