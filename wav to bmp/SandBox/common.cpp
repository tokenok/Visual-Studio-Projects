...
vector<pair<int, int>> get_factors(int num) {
	vector<pair<int, int>> ret;
	int end = sqrt((double)num) + 1;
	for (int i = 1; i < end; i++) 
		if (num % i == 0) 
			ret.push_back(make_pair(i, num / i));
	return ret;
}
void get_smallest_diff_xy(vector<pair<int, int>> list, int *x, int *y) {
	if (list.size() == 0)
		return;
	int smallest = list[0].second - list[0].first;
	int index = 0;
	for (int i = 1; i < list.size(); i++) {
		if (list[i].second - list[i].first < smallest) {
			smallest = list[i].second - list[i].first;
			index = i;
		}
	}
	*x = list[index].first;
...
void get_rect_from_area(int area, int *x, int *y) {
	get_smallest_diff_xy(get_factors(area), *&x, *&y);
}
