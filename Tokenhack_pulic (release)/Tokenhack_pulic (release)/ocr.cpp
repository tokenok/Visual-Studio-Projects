...
bool rgbt(COLORREF color) {
	return rgbt(GetRValue(color), GetGValue(color), GetBValue(color));
}
...
bool process(BYTE *newbuf, int x, int y, string outfile /* = "process.bmp"*/, bool OCR_visualization/* = false*/) {
	...
	for (int i = 0; i < y; i++) {
		for (int j = 0; j < 3 * x; j += 3) {
			...
			int green = pos + 1;
			int blue = pos + 2;
			if (!rgbt(newbuf[red], newbuf[green], newbuf[blue]) && ((OCR_visualization) ?
...
vector<string> ocr2(BYTE* newbuf, int x, int y, vector<vector<BYTE>>& charset) {
	vector<string> ret;
	...
	for (int i = 0; i < y; i++) {
		for (int j = 0; j < 3 * x; j += 3) {
			...
			if (rgbt(RGB(newbuf[pos], newbuf[pos + 1], newbuf[pos + 2])) && L[pos / 3] == 0) {
				L[pos / 3] = ++c;
				q.push(pos / 3);
				...
				while (!q.empty()) {
					...
					vector<int> v {k - x, k - x + 1, k + 1, k + x + 1, k + x, k + x - 1, k - 1, k - x - 1};
					for (int i = 0; i < v.size(); i++) {
						if (v[i] >= 0 && v[i] < (x * y)) {
							if (rgbt(RGB(newbuf[v[i] * 3], newbuf[v[i] * 3 + 1], newbuf[v[i] * 3 + 2])) && L[v[i]] == 0) {
								L[v[i]] = c;
								q.push(v[i]);
							}
						}
