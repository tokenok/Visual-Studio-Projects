...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*args*/, int /*iCmdShow*/) {
	HANDLE Mutex_handle = CreateMutex(NULL, TRUE, L"THE MUTEX OF POWER 2");
			...
			for (auto & a: normalbuttons) {
				if (a->id == LOWORD(wParam)) {
					...
					//		ShellExecute(hwnd, TEXT("open"), str_to_wstr(gamename).c_str(), str_to_wstr(targetlines).c_str(), str_to_wstr(gamedir).c_str(), SW_SHOW);
							...
							cmdline = str_to_LPWSTR(gamepath + " " + targetlines);

							if (!CreateProcess(0, cmdline, 0, 0, 0, 1024, 0, str_to_wstr(gamedir + "\\D2PK").c_str(), &si, &pi)) 
								printf("failed to start process: %s\n", error_code_to_text(GetLastError()).c_str());
							else 
...
*/
		...
		if (is_hotkey_dialog_visible) {
			...
			int x, y;
			BYTE* newbuf = windowcapture(GetForegroundWindow(), "capture.bmp", &x, &y);
		//	BYTE* newbuf = bmp_to_array("capture.bmp", x, y);
			BYTE* newbuf_ocr = new BYTE[x * 3 * y];
		//	BYTE* newbuf_ocr2 = new BYTE[x * 3 * y];
			for (int i = 0; i < y; i++) {
				for (int j = 0; j < 3 * x; j += 3) {
					int pos = i * x * 3 + j;
					newbuf_ocr[pos + 0] = newbuf[pos + 0];
		...
		//			newbuf_ocr2[pos + 0] = newbuf[pos + 0];
		//			newbuf_ocr2[pos + 1] = newbuf[pos + 1];
		//			newbuf_ocr2[pos + 2] = newbuf[pos + 2];
				}
			}
			
			vector<string> item_stats = ocr(newbuf_ocr, x, y, font16, true);
		//	vector<string> item_stats2 = ocr2(newbuf_ocr2, x, y, font16);
			process(newbuf_ocr, x, y, "(ocr)(processed)capture.bmp", true);
			process(newbuf, x, y, "(processed)capture.bmp", true);
			delete[] newbuf;
			delete[] newbuf_ocr;
