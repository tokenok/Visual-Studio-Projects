...
void OCR_auto_roll(HWND wnd, vector<vector<BYTE>> &font, POINT &item_pt) {
	...
	if (!get_transmute_pos(wnd, &transmutept)) {
		...
		if ((GetWindowLong(wnd, GWL_EXSTYLE) & WS_EX_LAYERED) && layered_window_fix)
	...
	if (item_stats.size() == 0) {
		...
		if (ocr_title_display) {
			...
			if (txt.find(windowtext) != string::npos)
				SetWindowText(wnd, str_to_wstr("#> -NO STATS FOUND- " + windowtext + " <# Diablo II").c_str());
		}
		if ((GetWindowLong(wnd, GWL_EXSTYLE) & WS_EX_LAYERED) && layered_window_fix)
			SetWindowLong(wnd, GWL_EXSTYLE, GetWindowLong(wnd, GWL_EXSTYLE) ^ WS_EX_LAYERED);
		return;
	...
	while (!match) {
		if (kill_all_OCR_auto_roll || find(auto_roll_windows.begin(), auto_roll_windows.end(), wnd) == auto_roll_windows.end() || !IsWindow(wnd)) {
			...
			if (ocr_title_display)
				SetWindowText(wnd, str_to_wstr("#> -KILLED- " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)((double)clock() - start_time)) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
			break;
		}
		...
		if (item_stats.size() == 0) {
			...
			if (no_stats_count > ocr_no_stats_timeout * 5) {
				if (ocr_title_display)
					SetWindowText(wnd, str_to_wstr("#> -EMERGENCY STOP- (NO STATS FOUND) " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)((double)clock() - start_time)) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
				break;
			}
			WINDOWPLACEMENT wp;
			...
			if (ocr_title_display)
				SetWindowText(wnd, str_to_wstr("#> -NO STATS FOUND- " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)((double)clock() - start_time)) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
		...
		if (data.prev_item_stats.size() == item_stats.size()) {
			if (compare_item_stats_to_file_stats(item_stats, data.prev_item_stats)) {
				...
				if (same_stats_count > ocr_same_stats_timout * 5) {
					if (ocr_title_display)
						SetWindowText(wnd, str_to_wstr("#> -EMERGENCY STOP- (SAME STATS FOUND) " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)((double)clock() - start_time)) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
					break;
				}
		...
		if (!match) {
			...
			if (ocr_title_display)
				SetWindowText(wnd, str_to_wstr("#> -ROLLING- " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)((double)clock() - start_time)) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
			this_thread::sleep_for(chrono::milliseconds(200));
			send_timer_click(item_pt, 0, 0, 0, wnd);
		}
	}
	...
	if (match) {
		...
		if (ocr_title_display)
			SetWindowText(wnd, str_to_wstr("#> -MATCH FOUND- " + windowtext + " Elapsed time: " + milliseconds_to_hms((int)total_time) + " Approx rolls: " + int_to_str(roll_count) + " <# Diablo II").c_str());
	}
	else {
		if (CTB_VOLUME.current_val > 0)
			PlaySound(MAKEINTRESOURCE(NOTFOUND), NULL, SND_ASYNC | SND_RESOURCE);
