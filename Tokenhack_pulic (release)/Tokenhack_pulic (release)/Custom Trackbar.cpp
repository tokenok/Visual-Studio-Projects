...
void custom_trackbar::redraw_newval(int val) {
	RECT tr = ctb_getclientrect(handle);
	width = tr.right - tr.left;
	current_val = val <= min_val ? min_val : val >= max_val ? max_val : val;
...
LRESULT CALLBACK customtrackbarProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_LBUTTONDOWN: {
			...
			PostMessage(GetParent(ctb->handle), WM_CUSTOM_TRACKBAR_VALUE_CHANGED, NULL, (LPARAM)ctb);
		...
		case WM_KILLFOCUS:{
			...
			for (auto & a: custom_trackbars) {
				if (a->focus) {
					...
					SendMessage(a->handle, WM_PAINT, NULL, NULL);
				}
			}
			break;
		}
