...
namespace TicTacToe {
		...
		int AIfindfork() {
			...
			for (int i = 0; i < r.Length; i++) {
				...
				for (int j = 1; j < r[i].GetLength(0); j += 2) {
					if (board[r[i][j, 0], r[i][j, 1]] == turn)
						a[(j - 1) / 2, 0]++;
					if (board[r[i][j + 1, 0], r[i][j + 1, 1]] == turn)
						a[(j - 1) / 2, 0]++;
					if (board[r[i][j, 0], r[i][j, 1]] == eturn)
						a[(j - 1) / 2, 1]++;
					if (board[r[i][j + 1, 0], r[i][j + 1, 1]] == eturn)
						a[(j - 1) / 2, 1]++;
				}
				int fork = 0;
	...
	class Program {
		static void Main(string[] args) {
			start:
			Console.ForegroundColor = ConsoleColor.White;
			Console.WriteLine("Left/Right Arrow to change menu options\nUp/Down Arrow to change their values\nPress Enter to play\n\n");
