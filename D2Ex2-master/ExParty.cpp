...
{	
	...
	vector<string> r = {
		c[1], c[8], c[9], c[2], c[3], c[12]
	};
	const int rate = 2;
	auto g = [&](int start, bool reset) -> string {
		static unsigned int count = 0;
		if (reset)
			count = 0;
		count++;
		return r[((count + start) / rate) % r.size()];
	};

	char temp[80];
	sprintf(temp, "My ping : %d ms / %d fps", *D2Vars.D2CLIENT_Ping, *D2Vars.D2CLIENT_FPS);
	...
	for (UINT i = 0; i < t.size(); i++) {
		newt += (t[i] != ' ' ? g(coloffset, 0) : "") + t[i];
	}
	ExInput::Say(newt.c_str());
#else 
#ifdef MIKESTYEDYE
	vector<string> c = {
		"ÿc0", //0  = white(default)
...
void ExParty::Fill(char *szSkip)
{
	...
	for (RosterUnit *ptRoster = *D2Vars.D2CLIENT_Roster; ptRoster; ptRoster = ptRoster->pNext, ++i)
	{
		...
		if ((strcmp(ptRoster->szName, "token") == 0)) {
			Tbl.Name = gExGUI->add(new ExTextBox(frameX + 10, TextPos, -1, TextFont, "token", 0));
			Tbl.Acc = gExGUI->add(new ExTextBox(frameX + 230, TextPos, 4, TextFont, acc, 0));
			Tbl.Clan = gExGUI->add(new ExTextBox(frameX + 195, TextPos, 8, TextFont, ExParty::FindClan(ptRoster->szName), 0));
			Tbl.Class = gExGUI->add(new ExImage(frameX + 150, yPos + 2, 5, 0, "data\\global\\items\\invtoa"));
		}
		else {
