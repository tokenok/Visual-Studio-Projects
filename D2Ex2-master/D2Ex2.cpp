...
BOOL D2Ex::Init()
{
	...
	BuffHKey = GetPrivateProfileInt("D2Ex", "BuffHKey", 0, ConfigIni.c_str());
	//WritePrivateProfileString("D2Ex", "BuffHKey", boost::lexical_cast<string>(BuffHKey).c_str(), ConfigIni.c_str());

	TransmuteHKey = GetPrivateProfileInt("D2Ex", "TransmuteHKey", VK_F10, ConfigIni.c_str());
	WritePrivateProfileString("D2Ex", "TransmuteHKey", boost::lexical_cast<string>(TransmuteHKey).c_str(), ConfigIni.c_str());
...
//	printf("%s\n%d\n%d\n", spath.c_str(), sum, ANNIHILUS_PATCH_D2_MPQ_CHECKSUM);
	...
	DisableMultiRes = sum < 104857600;//100Mb
	if (DisableMultiRes) { 
