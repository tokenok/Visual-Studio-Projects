...
void ExTextBox::Draw() {
	...
	if (cState == VISIBLE) {
		if (TextColor != -1) {
			...
			vector<string> r = {"ÿc1", "ÿc8", "ÿc9", "ÿc2", "ÿc3", "ÿc;"};

			string text(Label.begin(), Label.end());
			string atext = "";
			for (int i = 0; i < text.size(); i++)
				atext += r[(i + (color_count / 75)) % r.size()] + text[i];
			atext += '\0';

			wchar_t wName[65];
			char t[65];
			strcpy(t, atext.c_str());
			Misc::CharToWide(t, 65, wName, 65);
